...
public class BurpExtender implements BurpExtension {
    ...
    private int diy_ignore_switch = 0; // 新增：忽略规则开关
    ...
    // private JTextArea log_text; // 删除日志组件
    private JTextArea payload_jta;
    private JTextArea diy_error_jta;
    private JTextArea diy_ignore_jta; // 新增：忽略规则文本框
        ...
        String loadSuccess = """
                ========================================================================
                 ...
                 |_|  |_|_|    (_) |_|   \\___/|_|  \\___|_|\\__, |_| |_|\\___|_|  \s
                                                          |___/                \s
                [ SQLInjection v1.0 ] - [ LOAD SUCCESS! ]
                - Author: Mr.F0reigner
    ...
    private Component createUi() {
        ...
        tableSplitPane.setResizeWeight(0.7d); // 7:3 布局

        // 2. 配置面板 (GridBagLayout 实现极简间距)
        ...
        gbc.gridy++;
        gbc.insets = new Insets(3, 5, 5, 5); // 核心间距：上10px，下5px
        JTextField whiteTxt = new JTextField("");
        whiteTxt.setPreferredSize(new Dimension(100, 24)); // 锁定高度
        jps.add(whiteTxt, gbc);

        gbc.gridy++;
        gbc.insets = new Insets(0, 5, 5, 5); // 重置按钮间距
        JButton btnWhite = new JButton("启动白名单"); jps.add(btnWhite, gbc);

        gbc.gridy++; gbc.weighty = 1.0; // 推至顶端
        ...
        p_ctrl.add(chkCustomPay, pc); pc.gridy++; p_ctrl.add(chkSpaceEnc, pc); pc.gridy++; p_ctrl.add(chkValEmpty, pc); pc.gridy++; p_ctrl.add(btnLoadPay, pc);
        p_ctrl.setBorder(BorderFactory.createEmptyBorder(0, 0, 5, 0)); // 核心间距：下方5px
        ...
        e_ctrl.setBorder(BorderFactory.createEmptyBorder(0, 0, 5, 0)); // 核心间距：下方5px
        ...
        // --- 忽略报文模块 (新增) ---
        // 功能逻辑同报错特征：勾选=启用并锁定(禁编)，不勾选=关闭并允许编辑
        ...
        tab_diy.addTab("忽略报文", i_panel); // 替换了原来的日志
    ...
    private void performScan(HttpRequestResponse base, boolean force, String sourceName) {
        ...
        // 静态拦截
        if (url.toLowerCase().matches(".*\\.(jpg|png|gif|css|js|woff|pdf|mp4|ico|svg|jpeg|map)$"))
            return;
        if (white_switchs == 1 && Arrays.stream(white_URL.split(",")).noneMatch(w -> !w.isEmpty() && url.contains(w)))
            return;

        String hash = calculateMd5(base.request().method() + url + getParamKeys(base.request()));
        ...
        for (ParsedHttpParameter param : base.request().parameters()) {
            if (param.type() == HttpParameterType.URL || param.type() == HttpParameterType.BODY || param.type() == HttpParameterType.JSON || (checkCookie && param.type() == HttpParameterType.COOKIE)) {
                ...
                if (is_int == 1 && param.value().matches("\\d+")) {
                    payloads.add("-1");
                    payloads.add("-0");
                }
                if (JTextArea_int == 1) {
                    for (String dp : payload_jta.getText().split("\n")) {
                        if (!dp.trim().isEmpty())
                            payloads.add(diy_payload_1 == 1 ? dp.replace(" ", "%20") : dp);
                    }
                }
                for (String p : payloads) {
                    ...
                    det.parameter = param.name();
                    det.payload = finalVal;
    ...
    private String analyze(HttpRequestResponse base, HttpRequestResponse attack, String p, List<Pattern> errorRules, List<Pattern> ignoreRules) {
        ...
        if (diy_error_switch == 1) {
            for (Pattern pattern : errorRules) {
                if (pattern.matcher(body).find()) {
                    ...
                    if (r.toLowerCase().contains("mysql") || r.toLowerCase().contains("syntax") || r.contains("ora-") || r.toLowerCase().contains("postgresql"))
                        return "Err: " + r;
                    return "Debug: " + r;
                }
            }
        ...
        // 3. Diff 检测
        if (((p.equals("'") || p.equals("-1")) && base.response().body().length() != attack.response().body().length()))
            return "✔ Diff";
        if (((p.equals("''") || p.equals("-0")) && base.response().body().length() == attack.response().body().length()))
            return "✔ Recovered";
        return "Normal";
    }
    ...
    private String getParamKeys(HttpRequest req) {
        StringBuilder sb = new StringBuilder();
        for (ParsedHttpParameter p : req.parameters()) sb.append(p.name());
        return sb.toString();
    }
    ...
    private static class LogEntry {
        ...
        String status, parameter = "", payload = "", dataHash, source;
        long duration = 0;
    ...
    private class MasterTableModel extends AbstractTableModel {
        ...
        public String getColumnName(int c) {
            return new String[]{"#", "来源", "URL", "长度", "状态"}[c];
        }
        ...
        public Object getValueAt(int r, int c) {
            ...
            return switch (c) {
                case 0 -> e.id;
                case 1 -> e.source;
                case 2 -> e.requestResponse.request().url();
    ...
    private class DetailTableModel extends AbstractTableModel {
        ...
        public void updateData(String hash) {
            ...
            for (LogEntry de : detailLog) {
                if (de.dataHash != null && de.dataHash.equals(hash)) displayedDetail.add(de);
            }
        ...
        public LogEntry getEntryAt(int row) {
            return (row >= 0 && row < displayedDetail.size()) ? displayedDetail.get(row) : null;
        }

        @Override
        public int getRowCount() {
            return displayedDetail.size();
        }
        ...
        @Override
        public String getColumnName(int c) {
            return new String[]{"参数", "payload", "长度", "变化", "用时", "状态"}[c];
        }

        @Override
        public Object getValueAt(int r, int c) {
            LogEntry e = getEntryAt(r);
            if (e == null) return "";
            return switch (c) {
                case 0 -> e.parameter;
                case 1 -> e.payload;
                ...
                case 4 -> e.duration + "ms";
                case 5 -> e.requestResponse.response().statusCode();
                default -> "";
            };
        }
    }
    ...
    private class StatusCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable t, Object v, boolean is, boolean hf, int r, int c) {
            ...
            if (st.startsWith("Err:")) {
                ...
                bg = new Color(255, 230, 180);
                fg = new Color(255, 102, 0);
            } else if (st.contains("✔") || st.contains("end!")) {
                bg = new Color(204, 255, 204);
                fg = new Color(0, 128, 0);
            }
            if (is) {
                comp.setBackground(mix(bg, BURP_SEL));
    ...
    private class MyContextMenuProvider implements ContextMenuItemsProvider {
        @Override
        public List<Component> provideContextMenuItems(ContextMenuEvent event) {
            ...
            item.addActionListener(e -> {
                ...
                for (HttpRequestResponse rr : event.selectedRequestResponses())
                    executor.execute(() -> performScan(rr, true, src));
            });
